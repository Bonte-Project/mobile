...
class TrainerRepositoryImpl @Inject constructor(
    ...
    override suspend fun loadMyTrainer(): Result<TrainerResponse, DataError> =
        withContext(Dispatchers.IO) {
            safeCall<TrainerResponse> {
                httpClient.get("trainers/me")
            }.onSuccess { response ->
                val t = response.trainer
                val trainerEntity = response.trainer.toEntity()
                appDb.trainerDao.insert(listOf(trainerEntity))
                ...
                val trainerToExpList = t.experience.map { exp ->
                    ...
                    appDb.experienceDao.insert(listOf(expEntity))
    ...
    override suspend fun loadTrainers(): Result<TrainerListResponse, DataError> =
        withContext(Dispatchers.IO) {
            safeCall<TrainerListResponse> {
                httpClient.get("trainers")
            }.onSuccess { trainers ->
                appDb.trainerDao.insert(trainers.trainers.map { it.toEntity() })
                appDb.experienceDao.insert(trainers.trainers.flatMap { it.experience }
                    .map { it.toEntity() })
                appDb.trainerToExperienceDao.insert(trainers.trainers.flatMap { it.experience }
                    .map {
                        TrainerToExperienceEntity(
    ...
    override suspend fun addExperience(trainerId: String, request: ExperienceRequest): Result<TrainerResponse, DataError> =
        withContext(Dispatchers.IO) {
            safeCall<TrainerResponse> {
                ...
                val trainer = appDb.trainerDao.getTrainerById(trainerId = trainerId).first().trainerEntity
                ...
                appDb.experienceDao.insert(listOf(expEntity))
                appDb.trainerToExperienceDao.insert(
                    listOf(
                        TrainerToExperienceEntity(
                            trainer.trainerId,
                            expEntity.experienceId
                        )
                    )
                )
            }
    ...
    ): Result<TrainerResponse, DataError> =
    ...
    override suspend fun deleteExperience(
        trainerId: String,
        experienceId: String
    ): Result<TrainerResponse, DataError> =
        withContext(dbDeliveryDispatcher) {
            ...
            return@withContext when (deleteResult) {
                is Result.Success -> {
                    ...
                    val experiences = appDb.experienceDao.getExperience().first()
                        .filter { it.trainerId == trainerEntity.trainerId }
    ...
    override fun getTrainers(): Flow<List<Trainer>> =
        dbRepository
            .dbFlow
            .flatMapLatest { db ->  db.trainerDao.getTrainerList() }
            .flowOn(dbDeliveryDispatcher)
            .catch {
                it.printStackTrace()
            }

    override fun getTrainerFlowById(id: String): Flow<Trainer> =
        dbRepository.dbFlow
            .flatMapLatest { db -> db.trainerDao.getTrainerById(trainerId = id) }
            .flowOn(dbDeliveryDispatcher)
            .catch {
                it.printStackTrace()
            }
